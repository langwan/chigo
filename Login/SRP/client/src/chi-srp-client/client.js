...
function getx(params, salt, I, P) {
  ...
  var hashX = Buffer.from(
    crypto.createHash(params.hash).update(salt).update(hashIP).digest("hex"),
    "hex"
  );
...
function computeVerifier(params, salt, I, P) {
  ...
  assertIsBuffer(I, "identity (I)");
  assertIsBuffer(P, "password (P)");
  let x = getx(params, salt, I, P);

  var v_num = params.g.powm(x, params.N);
 ...
 */
function genKey() {
  return Buffer.from(randomHex(32), "hex");
}
...
function getu(params, A, B) {
  ...
  var u_buf = Buffer.from(
    crypto.createHash(params.hash).update(A).update(B).digest("hex"),
    "hex"
  );
...
function getK(params, S_buf) {
  ...
  return Buffer.from(
    crypto.createHash(params.hash).update(S_buf).digest("hex"),
    "hex"
  );
}
...
Client.prototype = {
  ...
  setPrivate: function (params, salt_buf, identity_buf, password_buf) {
    assertIsBuffer(salt_buf, "salt (salt)");
    assertIsBuffer(identity_buf, "identity (I)");
    assertIsBuffer(password_buf, "password (P)");
    this._private.x_num = getx(params, salt_buf, identity_buf, password_buf);
    console.log("x_num", this._private);
  },
